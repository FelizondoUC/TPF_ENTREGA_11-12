...
void capturar(GtkToggleButton *toggle_button, gpointer data) {
	...
	if(gtk_toggle_button_get_active(toggle_button)){
		switch (num) {
		case 1:
			if (board[capi - 1][capj] > 0 && board[capi - 1][capj] < 13) {
				temp1 = 1;
				botoncapmultipleFLAG++;
				if (board[capi - 1][capj] < 7) {
					arriba=board[capi-1][capj];
					sumcapmultiple= sumcapmultiple+arriba;
				}else {
					arriba=board[capi-1][capj] - 6;
					sumcapmultiple= sumcapmultiple+arriba;
				}
			}
			break;
		case 2:
			if (board[capi + 1][capj] > 0 && board[capi + 1][capj] < 13) {
				...
				botoncapmultipleFLAG++;
				if (board[capi + 1][capj] < 7) {
					abajo=board[capi+1][capj];
					sumcapmultiple= sumcapmultiple+abajo;
				}else {
					abajo=board[capi+1][capj] - 6;
					sumcapmultiple= sumcapmultiple+abajo;
				}
			}
			break;
		case 3:
			if (board[capi][capj - 1] > 0 && board[capi][capj - 1] < 13) {
				temp3 = 1;
				botoncapmultipleFLAG++;
				if (board[capi][capj - 1] < 7) {
					izquierda= board[capi][capj-1];
					sumcapmultiple= sumcapmultiple+izquierda;
				}else {
					izquierda= board[capi][capj-1] - 6;
					sumcapmultiple= sumcapmultiple+izquierda;
				}
			}
			break;
		case 4:
			if (board[capi][capj + 1] > 0 && board[capi][capj + 1] < 13) {
				...
				botoncapmultipleFLAG++;
				if (board[capi][capj + 1] < 7) {
					derecha=board[capi][capj+1];
					sumcapmultiple= sumcapmultiple+derecha;
				}else {
					derecha=board[capi][capj+1] - 6;
					sumcapmultiple= sumcapmultiple+derecha;
				}
			}
			break;
		...
		switch (num) {
		case 1:
			if (board[capi - 1][capj] > 1 && board[capi - 1][capj] < 13){
				temp1 = 0;
				arriba = board[capi - 1][capj];
				sumcapmultiple = sumcapmultiple - arriba;
			}
...
void conectar_captura(GtkWidget *widget, gpointer data)
{
	if (sumcapmultiple >=2 && sumcapmultiple < 7 && botoncapmultipleFLAG > 1 && opcion == 1){
		if (temp1 == 1){
			board[capi - 1][capj] = 0;
		}
		if (temp2 == 1){
	...
	board[capi][capj] = sumcapmultiple;
	if (colorP1 == 1) {
		for (int filas = 0; filas < 5; filas++) {
			for (int columnas = 0; columnas < 5; columnas++) {
				//obtenemos el estado de la posicion en la matriz
	...
	}else if (sumcapmultiple >=2 && sumcapmultiple < 7 && botoncapmultipleFLAG > 1 && opcion == 2){
		if (temp1 == 1){
					board[capi - 1][capj] = 0;
				}
				if (temp2 == 1){
			...
			board[capi][capj] = sumcapmultiple + 6;
			if (colorP1 == 1) {
				for (int filas = 0; filas < 5; filas++) {
					for (int columnas = 0; columnas < 5; columnas++) {
						//obtenemos el estado de la posicion en la matriz
	...
	temp3 = 0;
	temp4 = 0;
	gtk_toggle_button_set_active(Arriba, FALSE);
	gtk_toggle_button_set_active(Abajo, FALSE);
	gtk_toggle_button_set_active(Izquierda, FALSE);
	gtk_toggle_button_set_active(Derecha, FALSE);
	sumcapmultiple = 0;
	snprintf(buffer, sizeof(buffer), "La suma actual es: %d", sumcapmultiple);
	gtk_label_set_text(GTK_LABEL(suma_actual), buffer);
	snprintf(buffer,sizeof(buffer),"Abajo 0");
	gtk_label_set_text(GTK_LABEL(abajo_captura), buffer);
	botoncapmultipleFLAG = 0;
}
...
g_signal_connect(Capturar,"clicked",G_CALLBACK(conectar_captura),NULL);
